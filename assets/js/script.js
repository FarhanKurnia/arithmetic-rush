...
function calculateAnswer(questionString) {
    let expression = questionString;
    ...
    try {
        ...
        // Ganti pesan error internal menjadi "Jawaban tidak tersedia"
...
// --- FUNGSI JAWABAN ARITHMETIC RUSH ---
function showAnswer() {
    clearInterval(timerInterval); 
    const q = currentDataset[currentQuestionIndex];
    const calculatedAnswer = calculateAnswer(q.pertanyaan);
    const answerToDisplay = calculatedAnswer || 'Jawaban tidak tersedia'; 
    correctAnswerText.textContent = answerToDisplay; 
